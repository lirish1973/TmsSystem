...
if [ $? -eq 0 ]; then
    ...
    # שמירת המפתח הפרטי גם לקובץ נפרד לנוחות
    PRIVATE_KEY_FILE="github_deploy_private_key.txt"
    cat "$KEY_PATH" > "$PRIVATE_KEY_FILE"
    echo "💾 המפתח הפרטי נשמר גם ב-$PRIVATE_KEY_FILE לנוחיות"
    echo "⚠️  זכור למחוק את הקובץ הזה לאחר העתקה ל-GitHub!"
